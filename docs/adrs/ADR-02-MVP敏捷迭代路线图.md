...
### 阶段 5: MVP 3.0 (工作台工效、阅读连续性与拓扑分层排布 / Ergonomics, Persistence & Auto-Layout) —— [✅ 已完成，参见 ADR-03]
...
*   **核心突破**：
    1. **文献资产解耦与断点记忆持久化 (Document Decoupling & Reading Persistence)**：
       ...
       * **视窗状态与断点无损复原 (Zero-Jump Retention)**：抽屉切换与折叠时物理 `scrollTop` 零跳动，跨会话毫秒级精确还原阅读进度。
    2. **DAG 画布拓扑自动分层整理 (Sugiyama Auto-Layout & Anti-Overlap)**：
       * 引入 Sugiyama 分层排序与重心法减叉算法，配合包围盒动态碰撞消除，一键化解卡片重叠挤压；
       * 自左向右沿因果流向舒展排列，350ms 贝塞尔平滑过渡动画。
    3. **PDF 交互式章节目录大纲 (Interactive Outline & Halo Jump)**：
       * 递归解析 PDF 原生大纲书签目录，点击章节秒级卷动对齐目标页，并触发 1.5 秒科技感光效聚焦（Halo Effect）。
    4. **全局浅色模式 (Light Mode & Theme Persistence)**：
       * 适配日间学术高对比度阅读色彩体系，深浅主题一键切换与 LocalStorage 本地持久化。
*   **验收标准**：
