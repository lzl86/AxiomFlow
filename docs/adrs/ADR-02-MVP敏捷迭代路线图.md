...
### 阶段 5: MVP 3.0 (多文献跨论文交叉比对 / Cross-Paper Graph) —— [🚀 当前核心目标]
*   **主场模块**：`public/materials/`, `server.py`, `public/app.js`
*   **核心升级**：
    1. **多文献库管理 (Multi-Document Shelf)**：支持在阅读器中自由切换或并排对比多篇不同的相关文献（如：对比不同课题组对同一光学传递函数的近似解法）；
    2. **跨文献实证图谱融合 (Cross-Paper Fusion)**：允许将来自【论文 A P.12】与【论文 B P.88】的实证卡片同时连入同一个课题节点，由大模型进行交叉比对（Cross-Examination），自动挖掘理论冲突与学术空白。
*   **验收标准**：系统能同时容纳多份 PDF 资产，并在单一画布上完成跨文献的有向因果连线与学术异同辨析。

### 阶段 6: MVP 3.5 (数学与物理逆问题仿真求解 / Symbolic & Numeric Engine) —— [规划中]
...
### 阶段 7: MVP 4.0 (全自动化科研综述与学术 LaTeX 报告导出) —— [远期愿景]
*   **主场模块**：`exporter/latex_generator.py`
*   **核心升级**：
    1. **拓扑排序文章生成器**：根据画布上有向边的拓扑因果顺序（Topological Sort），将所有证据、推导与反思自动编排成连贯的论文章节大纲；
    2. **Overleaf / IEEE 标准模板注入**：一键导出包含 `\begin{equation}`、高清矢量图引用的标准 `.tex` 压缩包，直接可编译为学术论文初稿。
*   **验收标准**：点击“导出综述”，3 秒内生成排版完备的单篇学术报告 PDF/LaTeX。

### 阶段 8: MVP 5.0 (完全离线化本地开源大模型沙盒 / 零网络隐私计算) —— [终极目标]
*   **主场模块**：`local_engine/` (Ollama / vLLM / llama.cpp 集成)
*   **核心升级**：支持对接本地部署的开源视觉模型（如 InternVL2, Qwen2-VL），实现零外网调用、零数据泄漏的 100% 局域网物理安全隔离学术推演沙盒。
