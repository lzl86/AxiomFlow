...
*   **核心升级**：
    ...
    3. **DAG 规模化与因果推理深度演进 (Advanced DAG Scalability & Causal Reasoning Frontiers)**：
       * **自我中心聚焦透镜与非平面交叉抑制 (Ego-Graph Lens & Crossing Suppression)**：
         * 破解库拉托夫斯基非平面连线交叉困境（Crossing Number > 0），当卡片规模达到 50~100+ 时，激活自我中心模式：单选或悬停目标卡片时，画布自动虚化 90% 无关噪点，**仅高亮其 1-hop / 2-hop 祖先（依赖源头）与后继（影响范围）**，将视觉与前额叶工作记忆强制钳位在 $7 \pm 2$ 的生理极限内；
       ...
       * **边的逻辑算子强类型化 (Strongly-Typed Edges & Logic Gates: AND / OR / REFUTE)**：
         * 将单一无语义黑线升维为一阶逻辑门控：AND 汇聚桩（充要前置合取）、OR 分流桩（等价双轨析取分支）与 REFUTE 红色锯齿反驳弧（反例 Trace 显式证伪），使因果图跃迁为可视化一阶谓词证明网（Proof Net）；
       * **增量编译级脏状态与失效传播机制 (Reactive Invalidation & Knowledge CI/CD)**：
         * 借鉴 Rustc Salsa / Bazel 细粒度增量编译架构：当下游依赖的上游公理、底层代码或微部件参数发生 Diff 时，下游路径所有受影响卡片**自动浮现黄色虚线警示指示器（State: Stale/Dirty）**，直至下游卡片重新执行内部验证断言通过后自动转绿，达成学术推演资产的持续集成；
...
*   **验收标准**：
    ...
    4. 修改上游核心参数或断言后，下游依赖节点毫秒级捕获失效事件并呈现黄色脏标记，重新验证通过后平滑复原。
...
### 约束 9：文本主线与微部件局部除颤法则 (Text-Dominant Baseline & Micro-Widget Defibrillator Invariant)
* **教训总结**：
  1. 只要没有遭遇认知阻塞，文本（结合代码与形式化符号）永远是人类前额叶直接解码吞吐速率最高、阻抗最低的“第一媒介”；
  2. 盲目将所有概念都做成动画或交互微部件，不仅不会提升效率，反而会因巨大的机械阻抗（鼠标拖动、时间锁相、隐喻泄漏）让系统学习沦为儿童游戏化科普；
  3. **认知阻抗的非线性分流**：卡壳并非单一维度的“难”，必须严格区分为“宏观因果断层”与“微观状态爆炸”两类正交故障，严禁无差别堆砌工具。
* **强制规范**：
  1. **85/15 媒介分工铁律**：日常 85%~90% 的工程学习与知识沉淀必须以纯净高密度的 Markdown、C/C++ 源码、RFC 契约和形式化数学推导为主线；
  2. **二维故障诊断分流（详见 [METHODOLOGY-认知工效与分流诊断操作指南.md](../METHODOLOGY-认知工效与分流诊断操作指南.md)）**：
     * **故障 A（宏观因果断层 / 前置迷航）**：呼叫 **AxiomFlow DAG**，执行最小祖先闭包回溯与可传递归约剪枝，理清因果充要推导链，严禁滥用微部件；
     * **故障 B（微观状态空间爆炸 / 时序并发撕裂）**：呼叫 **可交互微部件 (Micro-Widget)**，通过 60FPS 连续刮擦与 V1 撞色快速建立感官直觉；
  3. **微部件的除颤仪定位与登顶之梯**：可交互微部件严格限制在 10%~15% 的微观状态空间爆炸断面；在 1~3 分钟内打通物理直觉后，**必须通过单击下钻（Click-to-Drill-Down）锚定回源码精确行号与物理内存地址，随即立刻关闭微部件、踢开梯子**，回归高密度文本与代码断言。

### 约束 10：DAG 因果纯度、逻辑算子与防通胀铁律 (DAG Semantic Purity, Logic Gates & Anti-Inflation Invariant)
* **教训总结**：
  1. DAG 在像素表面积上显得稀疏，但在语义信息论上是对一维线性文本的极限压缩（消灭了数百字的序列化胶水与上下文复述冗余）；
  2. 若缺乏严格图论纪律，DAG 极易退化为三种伪图反模式：**画框狂热**（把三行代码拆成 5 张空洞卡片）、**弱语义关联线**（把边当成无因果的联想线）与**巨石大包袱卡片**（卡片内塞 5000 字长文破坏图计算）。
* **强制规范**：
  1. **严格因果与计算契约**：AxiomFlow 的有向边仅允许承载两种语义：**充要逻辑推导**（必要前置条件）或**反应式参数传递总线**（数据流输入输出），严禁建立任何无因果约束的“参考/相关”废边；
  2. **拓扑传递归约 (Transitive Reduction)**：图引擎必须保证因果图的最小等价性，若存在 $A \to B \to C$，则 $A \to C$ 视为视觉与逻辑冗余并强制归约；
  3. **卡片原子性底线**：一张卡片必须且只能对应一个独立的公理、结构体定义、状态机或形式化定理，脱离上下文无法自证闭环的内容严禁独立成卡；
  4. **渐进式逻辑算子升级**：边的汇聚必须明确布尔代数语义（AND 合取前置 / OR 析取分支 / REFUTE 证伪反驳），严禁将多入边做成语义含混的黑盒依赖。
