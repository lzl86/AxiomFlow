...
* **强制规范**：
  ...
  2. **微部件的除颤仪定位**：可交互微部件严格限制在 10%~15% 的“前额叶状态空间爆炸断面”（如流水线冒险冲突、Cache 伪共享震荡、盘口深度非线性塌缩）；
  3. **用完即弃的登顶之梯**：微部件仅用于在 1~3 分钟内利用感觉皮层快速打通物理直觉，一旦直觉形成并通过单击下钻确认源码行，必须立刻关闭微部件、剔除梯子，回归高密度文本与代码断言。
